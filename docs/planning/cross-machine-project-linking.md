...
| 4 | Refresh trigger | App-bootstrap backfill for bindings missing `git_remote_url` whose folder exists on this machine. Panel "Refresh git link" stays as a one-off if origin changes later. No watcher, no periodic re-scan. | Existing rows predate the column. Waiting for a per-card button means the Projects page never groups until the user notices. Startup is the one time we already walk local state. |
