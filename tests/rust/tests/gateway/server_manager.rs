...
async fn test_get_status_returns_connecting_after_enable() {
    ...
    assert!(matches!(conn_status, mcpmux_gateway::pool::ConnectionStatus::Connecting));
}
...
async fn test_get_all_statuses_for_space() {
    ...
    let key1 = ServerKey { space_id, server_id: "server-1".to_string() };
    let key2 = ServerKey { space_id, server_id: "server-2".to_string() };
...
async fn test_disable_increments_flow_id() {
    ...
    assert!(flow_after > flow_before, "Flow ID should be incremented on disable");
}
...
async fn test_enable_emits_server_status_changed() {
    ...
    assert!(status_changed.is_some(), "Should emit ServerStatusChanged event");
}
...
async fn test_event_contains_correct_space_id() {
    let mut harness = ServerManagerTestHarness::new().await;
    let space_id = Uuid::new_v4();
    let key = ServerKey { space_id, server_id: "server-1".to_string() };
...
async fn test_enable_assigns_prefix() {
    let harness = ServerManagerTestHarness::new().await;
    let space_id = Uuid::new_v4();
    let key = ServerKey { space_id, server_id: "test-server".to_string() };
    
    harness.manager.enable_server(key).await.unwrap();
    
    // Check prefix was assigned
    let prefix = harness.prefix_cache
...
async fn test_disable_releases_prefix() {
    ...
    let available = harness.prefix_cache
...
async fn test_multiple_servers_independent() {
    ...
    assert!(matches!(status1.0, mcpmux_gateway::pool::ConnectionStatus::Connecting));
    assert!(matches!(status2.0, mcpmux_gateway::pool::ConnectionStatus::Connecting));
}
...
fn assert_event_status(events: &[DomainEvent], expected_server_id: &str, expected_status: ConnectionStatus) {
    ...
    assert!(found, "Expected event with server_id={} and status={:?}", expected_server_id, expected_status);
}
